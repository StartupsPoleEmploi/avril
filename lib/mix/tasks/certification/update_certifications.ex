defmodule Mix.Tasks.UpdateCertifications do
  ...
  defp merge_certifications(certification) do
    ...
    if Enum.member?(certification_ids, keep_certification_id) do
      ...
      Enum.each(remove_certification_ids, fn remove_certification_id ->
        association_remove_dups_and_update_others(keep_certification_id, remove_certification_id, :certifiers, "certifier_certifications")
        association_remove_dups_and_update_others(keep_certification_id, remove_certification_id, :romes, "rome_certifications")
        association_remove_dups_and_update_others(keep_certification_id, remove_certification_id, :delegates, "certifications_delegates")
      end)
  ...
  def association_remove_dups_and_update_others(keep_certification_id, remove_certification_id, association, table_name) do
    keep_certification_association_ids = Vae.Repo.get(Vae.Certification, keep_certification_id) |> Vae.Repo.preload(association) |> Map.get(association) |> Enum.map(&(&1.id))
    remove_certification_association_ids = Vae.Repo.get(Vae.Certification, remove_certification_id) |> Vae.Repo.preload(association) |> Map.get(association) |> Enum.map(&(&1.id))

    update_association_ids = remove_certification_association_ids -- keep_certification_association_ids
    ...
    if length(update_association_ids) > 0 do
      result = Ecto.Adapters.SQL.query!(
        Vae.Repo, "UPDATE #{table_name} SET certification_id=$1 WHERE certification_id=$2 AND #{association |> Inflex.singularize()}_id IN (#{update_association_ids |> Enum.join(", ")})", [keep_certification_id, remove_certification_id]
      )
      IO.puts("#{result.num_rows} #{table_name} mises à jour")
    end
    if length(remove_association_ids) > 0 do
      result = Ecto.Adapters.SQL.query!(
        Vae.Repo, "DELETE FROM #{table_name} WHERE certification_id=$1 AND #{association |> Inflex.singularize()}_id IN (#{remove_association_ids |> Enum.join(", ")})", [remove_certification_id]
      )
  ...
  def smart_find_by_slug(slug) do
    Repo.get_by(Vae.Certification, slug: slug) || (
      ...
      wheres = Enum.reduce(words, true, fn word, query ->
