defmodule VaeWeb.ResetPasswordController do
  ...
  def update(conn, %{"id" => token, "user" => user_params}) do
    ...
    case PowResetPassword.Plug.update_user_password(conn, user_params) do
      {:ok, _user, conn} ->
        conn
        |> put_flash(:info, "Votre mot de passe a bien été mis à jour, vous pouvez vous connecter.")
  ...
  defp redirect_to_home(conn) do
    conn
    |> put_flash(:info, "Un email vous a été envoyé. Nous vous invitons à consulter votre boîte de réception.")
  ...
  defp load_user_from_reset_token(%{params: %{"id" => token}} = conn, _opts) do
    case PowResetPassword.Plug.load_user_by_token(conn, token) do
      {:error, conn} ->
        conn
        |> put_flash(:error, "Le lien que vous avez cliqué n'est pas valide. Merci de réinitialiser à nouveau votre mot de passe.")
