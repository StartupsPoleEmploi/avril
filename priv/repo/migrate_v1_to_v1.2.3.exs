...
|> Enum.each(fn certifier ->
  ...
  |> Enum.each(fn delegate ->
    processes_steps =
      Enum.reduce(1..8, [], fn n, acc ->
        with step when step != nil <- Map.get(delegate, :"step_#{n}"),
             title <- Floki.find(step, "h3") |> Floki.text(),
             content <- Floki.filter_out(step, "h3") |> Floki.raw_html() do
          step =
            case Repo.get_by(Step, content: content) do
              nil ->
                Repo.insert!(%Step{title: title, content: content},
    ...
    process =
      Repo.insert!(%Process{name: delegate.name, processes_steps: processes_steps},
