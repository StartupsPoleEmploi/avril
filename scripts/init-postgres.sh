...
if createdb -U $POSTGRES_USER -w $POSTGRES_DB 2> /dev/null; then
...
if psql -U $POSTGRES_USER -d $POSTGRES_DB -c 'select count(*) from schema_migrations;'; then
# if psql -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -c 'select count(*) from schema_migrations;'; then
  ...
  if [[ -f $DUMP_FILE ]]; then
    ...
    pg_restore --verbose --clean --no-acl --no-owner -d $POSTGRES_DB -U $POSTGRES_USER -w $DUMP_FILE || true
    echo "Checking restore";
    psql -U $POSTGRES_USER -d $POSTGRES_DB -c 'select count(*) from schema_migrations;' || true
  else
